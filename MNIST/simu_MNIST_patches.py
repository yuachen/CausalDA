...
parser.add_argument("--perturb", type=str, default="whitepatch", help="type of perturbation")
...
parser.add_argument("--lamCIPMMD", type=float, default=0.1, help="CIP matching penalty with MMD")
parser.add_argument("--epochs", type=int, default=50, help="number of epochs")
parser.add_argument("--tag_DA", type=str, default="baseline", help="whether to run baseline methods or DA methods")
parser.add_argument("--M", type=int, default=12, help="total number of environments")
...
savefilename_prefix = 'results_MNIST/report_v8_%s_M%d_subsetprop%s_%s_lamMatch%s_lamCIP%s_lamMatchMMD%s_lamCIPMMD%s_epochs%d_seed%d' % (myargs.perturb, M, 
                           str(myargs.subset_prop), myargs.tag_DA, str(myargs.lamMatch), str(myargs.lamCIP), str(myargs.lamMatchMMD),
                           str(myargs.lamCIPMMD), myargs.epochs, myargs.seed)
...
if myargs.tag_DA == 'baseline':
    ...
    methods = [
               ...
               semitorchMNISTclass.DIPOracle(lamMatch=myargs.lamMatch, lamL2=0., lamL1=0., 
                                         sourceInd = 0, lr=lr, epochs=myargs.epochs, wayMatch='mean'),
               semitorchMNISTclass.DIPweigh(lamMatch=myargs.lamMatch, lamL2=0., lamL1=0., 
                                         lr=lr, epochs=myargs.epochs, wayMatch='mean'),
               semitorchMNISTclass.CIRMweigh(lamMatch=myargs.lamMatch, lamCIP=myargs.lamCIP, lamL2=0., lamL1=0.,
                                                lr=lr, epochs=myargs.epochs, wayMatch='mean'),
              ]
    ...
    methods = [
               ...
               semitorchMNISTclass.DIPweigh_MMD(lamMatch=myargs.lamMatchMMD, lamL2=0., lamL1=0., 
                                         lr=lr, epochs=myargs.epochs, wayMatch='mmd', sigma_list=[1., 10.]),
               semitorchMNISTclass.CIRMweigh_MMD(lamMatch=myargs.lamMatchMMD, lamCIP=myargs.lamCIPMMD, lamL2=0., lamL1=0.,
                                                lr=lr, epochs=myargs.epochs, wayMatch='mmd', sigma_list=[1., 10.]),
              ]
elif myargs.tag_DA == 'DACIPMMD':
    methods = [
               semitorchMNISTclass.CIP_MMD(lamCIP=myargs.lamCIPMMD, lamL2=0., lamL1=0.,
                                                lr=lr, epochs=myargs.epochs, wayMatch='mmd', sigma_list=[1., 10.]),
              ]
elif myargs.tag_DA == 'DACIPmean':
    methods = [
               semitorchMNISTclass.CIP(lamCIP=myargs.lamCIP, lamL2=0., lamL1=0.,
                                                lr=lr, epochs=myargs.epochs, wayMatch='mean'),
              ]
else:
    print('tag_DA unrecognized')
...
for i, me in enumerate(methods):
    ...
    if hasattr(me, 'losses'):
        print(me.losses, file=savefile)
    if hasattr(me, 'minDiffIndx'):
        print("best index="+str(me.minDiffIndx), file=savefile)
